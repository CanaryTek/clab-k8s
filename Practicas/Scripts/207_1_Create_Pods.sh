...
ssh node01 "sudo cat /var/lib/kubelet/config.yaml.bck | sed 's/kubernetes\/manifests/para_complicarlo/' | sudo tee /var/lib/kubelet/config.yaml >/dev/null && sudo systemctl restart kubelet"
